...
const cacheFirst = async ({ request, preloadResponsePromise, bypassCache }) => {
  if (!bypassCache) {
  ...
  try {
    ...
    if (!bypassCache && responseFromNetwork && responseFromNetwork.ok && responseFromNetwork.type === 'basic') {
...
function shouldBypassAppCache(request) {
  try {
    ...
    return new URLSearchParams(hash.slice(qIndex + 1)).get('fresh') === '1';
  } catch (e) {
    return false;
  }
}
...
self.addEventListener('fetch', (event) => {
  ...
  event.respondWith(
    cacheFirst({
      ...
      bypassCache: shouldBypassAppCache(event.request),
    })
  );
});
