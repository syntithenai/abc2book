"""OCR chord symbols above staves and align into melody ABC quote-chords."""
...
from sheet_image_abc_repair import looks_weak_abc
...
def normalize_transpose_in_abc(abc: str) -> str:
    """Passthrough for chord overlay output (legacy transpose markers rare on OMR)."""
    return abc or ""
...
def try_chord_ocr_overlay(
    image_path: str,
    base_abc: str,
    *,
    staff_info: dict[str, Any] | None = None,
    ocr_boxes: list[dict[str, Any]] | None = None,
    enhanced_omr: dict[str, Any] | None = None,
) -> tuple[str | None, dict[str, Any]]:
    """Return chorded ABC + status, or (None, status) when gates fail."""
    abc = str(base_abc or "").strip()
    if not abc or looks_weak_abc(abc):
        return None, {"reason": "weak-base-abc"}

    info = staff_info or detect_staff_regions(image_path)
    bands = list(info.get("staffRegions") or [])
    boxes = list(ocr_boxes or [])

    image_height = 0.0
    if bands:
        image_height = max(float(b.get("bottom") or 0) for b in bands) + 40.0
    for b in boxes:
        image_height = max(
            image_height,
            float(b.get("y") or 0) + float(b.get("height") or 0),
        )
    ...
    system_hint = None
    system_bar_counts = None
    if isinstance(enhanced_omr, dict):
        ok = enhanced_omr.get("okSystems")
        if ok is not None:
            try:
                system_hint = int(ok)
            except (TypeError, ValueError):
                system_hint = None
        raw_counts = enhanced_omr.get("barsPerSystem")
        if isinstance(raw_counts, list) and raw_counts:
            system_bar_counts = [max(1, int(c)) for c in raw_counts]
