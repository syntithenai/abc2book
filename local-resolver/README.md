...
| POST | `/voice-command` | Voice command: upload short audio (Whisper) **or** send a `transcript` form field from on-device STT, then parse SHOW/SEARCH intent (regex fast path + LLM) and return a structured tool call |
