...
function App(props) {
  ...
  var tunebook = useTuneBook({startWaiting, stopWaiting, setPageMessage,isMobile, currentTune, setCurrentTune})
  return (
      ...
      <Router >
        ...
        <div className="App-body">
            <Routes>
              ...
              <Route  path={`print`}   element={<PrintPage  tunebook={tunebook}    />}  />
              <Route  path={`review`}   element={<ReviewPage  tunebook={tunebook}    />}  />
              <Route  path={`menu`}   element={<MenuPage  tunebook={tunebook}    />}  />
              <Route  path={`tunes`}     >
                <Route
                  index
                  element={<MusicPage  tunebook={tunebook} currentTuneBook={currentTuneBook} setCurrentTuneBook={setCurrentTuneBook}  />}
                />
                <Route  path={`:tuneId`} element={<MusicSingle forceRefresh={forceRefresh} tunebook={tunebook}    />} />
              </Route>
              
              <Route  path={`editor`}     >
                <Route  path={`:tuneId`} element={<MusicEditor forceRefresh={forceRefresh} tunebook={tunebook}    />} />
              </Route>
                
            </Routes>
        </div>
        <div style={{backgroundColor:'#e8f8fe', height:'6em'}} >
          <Link to='/help' ><Button>Help</Button></Link>
          <div style={{textAlign:'center'}}>(CopyLeft 2022)    Steve Ryan <a href='mailto:syntithenai@gmail.com'>syntithenai@gmail.com</a></div>
          <div style={{textAlign:'center'}}>Source code on <a href='https://github.com/syntithenai/abc2book'>Github</a></div>
        </div>
      </Router>
