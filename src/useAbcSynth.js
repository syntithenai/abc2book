...
import { scrollPlayalongPlayingLineIntoCenter } from './playalongStaffLayout'
...
import { programOffsetsForPlaybackPlan, abcjsPlaybackSynthOptions } from './abcSynthProgramOffsets'
...
import { resolveFillPlaybackOptions, FILL_STYLE_OFF } from './playbackFillSettings'
import { buildPlaybackSequence, barWholeNotesFromMeter } from './playbackFillPattern'
import { buildSoundingWrittenMap, expandVisualObjThroughVoltaRepeats, mapSoundingWholeToWritten } from './voltaRepeatExpand'
...
import { audioTimeForGlobalSlot, reanchorTimelineAtSlot } from './rhythmTimeline'
...
export default function useAbcSynth(props) {
    ...
    /** Suppress "Could not load notation" after stop/invalidate cancels an in-flight prime. */
    const midiPrimeCancelLatchUntilRef = useRef(0)
    /** Avoid re-entering natural end / restart loops from timing callbacks. */
    const playbackFinishedRef = useRef(false)
    const deferredNaturalEndTimerRef = useRef(null)
    const deferredNaturalEndAttemptRef = useRef(0)
    /** Avoid startMidiAndTiming → startPlaying → startMidiAndTiming retry loops. */
    const midiAudioRestartAfterFailRef = useRef(false)
    const playbackTimingMapRef = useRef(null)
    const lastPlaybackTimingSampleRef = useRef(null)
    const soundingWrittenMapRef = useRef(null)
    const effectiveMsPerMeasureRef = useRef(0)
    const primedMsPerMeasureRef = useRef(0)
    ...
    async function ensureSynthAudioContextRunning() {
        ...
        const fromUserGesture = (props.hasPlaybackGesture && props.hasPlaybackGesture())
            || (props.mediaController && props.mediaController.userGesturePlayRef
        ...
        if (gaudioContext.current.state !== 'running' && fromUserGesture) {
            try {
                await gaudioContext.current.resume()
            } catch (e) {
                return false
    ...
    function needsMidiAudioContextUnlock() {
        if (!abcjs.synth.supportsAudio()) return true
        const ctx = gaudioContext.current
        if (!ctx) return true
        return ctx.state === 'suspended' || ctx.state === 'interrupted'
    }

    function latchMidiPrimeCancel(ms) {
        midiPrimeCancelLatchUntilRef.current = Date.now() + (ms > 0 ? ms : 2500)
        if (props.mediaController && props.mediaController.latchMidiPrimeQuiet) {
            props.mediaController.latchMidiPrimeQuiet(ms > 0 ? ms : 2500)
        }
    }

    function shouldQuietMidiPrimeFailure() {
        if (Date.now() < midiPrimeCancelLatchUntilRef.current) return true
        if (primePromiseRef.current) return true
        return false
    }

    function reportMidiPlaybackFailure() {
        releaseMidiUiLoading()
        setIsPlaying(false)
        if (getForceStop() || shouldQuietMidiPrimeFailure()) {
            if (props.mediaController && props.mediaController.setIsPlaying) {
                props.mediaController.setIsPlaying(false)
            }
            return
        }
        // Another prime superseded this one without bumping generation.
        if (primePromiseRef.current || midiPrimeInFlightRef.current) {
            if (props.mediaController && props.mediaController.setIsPlaying) {
                props.mediaController.setIsPlaying(false)
            }
            return
        ...
        if (props.mediaController) {
            props.mediaController.setIsPlaying(false)
            if (needsMidiAudioContextUnlock()) {
                showMidiTapToPlay()
                return
            }
            if (props.mediaController.reportNotationPrimeFailure) {
                props.mediaController.reportNotationPrimeFailure()
                return
            }
        }
        if (needsMidiAudioContextUnlock()) {
    ...
    function clearPlaybackFinished() {
        playbackFinishedRef.current = false
    }

    function clearDeferredNaturalEndCheck() {
        if (deferredNaturalEndTimerRef.current) {
            clearTimeout(deferredNaturalEndTimerRef.current)
            deferredNaturalEndTimerRef.current = null
        }
    }

    function clearDeferredNaturalEndRecovery() {
        clearDeferredNaturalEndCheck()
        deferredNaturalEndAttemptRef.current = 0
    }

    function scheduleDeferredNaturalEndCheck(delayMs) {
        clearDeferredNaturalEndCheck()
        deferredNaturalEndAttemptRef.current += 1
        if (deferredNaturalEndAttemptRef.current > 40) return
        deferredNaturalEndTimerRef.current = setTimeout(function() {
            deferredNaturalEndTimerRef.current = null
            if (shouldIgnoreMidiPlaybackCompletion({
                playbackFinished: playbackFinishedRef.current,
                deferToNative: shouldDeferSynthStopToNative(),
                guardActive: Date.now() < midiPlaybackGuardUntilRef.current,
            })) {
                return
            }
            if (isMidiAudioNearNaturalEnd()) {
                deferredNaturalEndAttemptRef.current = 0
                handlePlaybackCompletion()
            } else {
                scheduleDeferredNaturalEndCheck(150)
            }
        }, Math.max(0, delayMs || 0))
    }

    function clearPendingRepeatRestart() {
    ...
    /** TimingCallbacks can reach totalBeats before the audible buffer ends (two-part
     *  tunes, fill/tempo skew). Do not finish or Abc-loop until audio is near the end. */
    function isMidiAudioNearNaturalEnd() {
    ...
    function finishNaturalPlayback(endRewindOpts) {
        ...
        clearPendingRepeatRestart()
        if (props.mediaController && props.mediaController.disarmPlaybackIntent) {
            props.mediaController.disarmPlaybackIntent()
        }
        stopPlaying()
        if (endRewindOpts && !shouldSuppressPlaybackEndSeek()) {
            seekPlayer(0, false, endRewindOpts)
        }
    ...
    function handlePlaybackCompletion() {
        ...
        if (!isMidiAudioNearNaturalEnd()) {
            scheduleDeferredNaturalEndCheck(0)
            return
        }
        clearDeferredNaturalEndRecovery()

        const endRewindOpts = { skipAutoResume: true }
        const repeat = parseInt(props.repeat, 10)
        if (repeat === -1) {
            if (shouldSuppressPlaybackEndSeek() || props.forceFillOff) {
                finishNaturalPlayback(null)
            } else {
                clearPlaybackFinished()
                setForceStop(false)
                scheduleRepeatRestart()
            }
        } else if (repeat === 0) {
            finishNaturalPlayback(endRewindOpts)
        } else if (repeat > 0) {
            if (shouldScheduleMidiRepeatRestart(repeat, playCountRef.current, props.forceFillOff)) {
                incrementPlayCount()
                ...
                clearPlaybackFinished()
                scheduleRepeatRestart()
            } else if (playCountRef.current < repeat - 1) {
                setPlayCount(0)
                finishNaturalPlayback(null)
            } else {
                setPlayCount(0)
    ...
    function handleMidiAudioStartFailure() {
        // Count-in can leave countInPending / rhythm handoff set while audio never
        // started; without clearing these, tap-to-play retries no-op in beginMidiPlayback.
        ...
        if (needsMidiAudioContextUnlock()) {
            showMidiTapToPlay()
        } else if (props.mediaController && props.mediaController.reportNotationPrimeFailure) {
            props.mediaController.reportNotationPrimeFailure()
        } else {
            showMidiTapToPlay()
        }
    ...
    function stopNativeMidiBufferOutput() {
        ...
        try {
            ...
            // stop() tears down the AudioBufferSource and forces a full re-prime
            // (which races generation bumps and toasts "Could not load notation").
            if (gmidiBuffer.current.pause) {
                gmidiBuffer.current.pause()
            } else if (gmidiBuffer.current.isRunning) {
                gmidiBuffer.current.stop()
            }
        } catch (e) {}
    }

    function resolveMidiConnectAudioTime(startAtAudioTime) {
        const ctx = gaudioContext.current
        if (!ctx || !Number.isFinite(startAtAudioTime)) {
            return null
        }
        const now = ctx.currentTime
        // Only snap to now when the scheduled start is already past (or within
        // tiny scheduling jitter). Pulling future starts forward by up to 25ms
        // made notes lead the metronome; starting late without re-anchoring
        // left clicks ~10ms early vs the audible notes.
        if (startAtAudioTime <= now + 0.002) {
            return now
        }
        return startAtAudioTime
    }
    ...
    function startMidiAudioOutput(settings, ratio, startAtAudioTime) {
        ...
        const startRatio = typeof ratio === 'number' ? ratio : 0
        // Starting within the last 1% only re-triggers an immediate end and,
        // with the playback guard, a reconnect loop (observed ratio≈0.9978).
        if (startRatio >= 0.99 && !Number.isFinite(startAtAudioTime)) {
            return { ok: false, actualStartAudioTime: null }
        }
        const ctx = gaudioContext.current
        const when = resolveMidiConnectAudioTime(startAtAudioTime)
        const targetAudible = Number.isFinite(startAtAudioTime) ? startAtAudioTime : null
        if (pitchShifterRef.current) {
            ...
            // Reuse an in-flight connect aimed at the same instant (count-in
            // pre-schedule). Reconnecting here tore it down and restarted late.
            if (pitchShifterRef.current.isConnectedOrPending
                && pitchShifterRef.current.isConnectedOrPending()) {
                const existing = typeof pitchShifterRef.current.getScheduledConnectTime === 'function'
                    ? pitchShifterRef.current.getScheduledConnectTime()
                    : null
                if (typeof existing === 'number' && typeof targetAudible === 'number'
                    && Math.abs(existing - targetAudible) < 0.025) {
                    return { ok: true, actualStartAudioTime: existing }
                }
                // Already audible: do not tear down/reconnect for a same-position
                // start — that caused first-line seek thrash on /playMidi load.
                if (pitchShifterRef.current.isConnected() && targetAudible == null) {
                    const live = typeof pitchShifterRef.current.getCurrentTime === 'function'
                        ? pitchShifterRef.current.getCurrentTime()
                        : -1
                    const dur = pitchShifterRef.current.duration > 0
                        ? pitchShifterRef.current.duration
                        : 0
                    const liveRatio = dur > 0 && live >= 0 ? live / dur : -1
                    const samePos = liveRatio >= 0 && Math.abs(liveRatio - startRatio) < 0.01
                    // forceRatio:0 while already past the opening is a repeated
                    // auto-start, not a user rewind — keep the running buffer.
                    const spuriousRestartFromZero = startRatio < 0.01 && liveRatio > 0.02
                    if (samePos || spuriousRestartFromZero) {
                        pitchShifterRef.current.applySettings(settings.tempo, settings.pitch, settings.fineTune)
                        return { ok: true, actualStartAudioTime: existing != null ? existing : (ctx ? ctx.currentTime : null) }
                    }
                }
            }
            ...
            const connected = pitchShifterRef.current.isConnectedOrPending
                ? pitchShifterRef.current.isConnectedOrPending()
                : pitchShifterRef.current.isConnected()
            // Prefer the scheduled Web Audio start (not now) so late callers
            // that still pass a future `when` report the true audible time.
            const scheduledConnect = typeof pitchShifterRef.current.getScheduledConnectTime === 'function'
                ? pitchShifterRef.current.getScheduledConnectTime()
                : null
            const actualStart = typeof scheduledConnect === 'number'
                ? scheduledConnect
            ...
            return connected
                ? { ok: true, actualStartAudioTime: actualStart }
    ...
    function wantsMidiPlayback(force) {
        if (props.mediaController) {
            const midiRoute = !!(props.mediaController.isMidiPlaybackRoute
                && props.mediaController.isMidiPlaybackRoute())
            if (!midiRoute) {
                if (props.mediaController.requestedPlayState === 'playMedia') {
                    return false
    ...
    const lastPrimedAudioHashRef = useRef(null)
    ...
    useEffect(function() {
        ...
        const midiRouteActive = mc.isMidiPlaybackRoute && mc.isMidiPlaybackRoute()
        const mediaInFlight = !midiRouteActive
            && mc.isLinkedMediaPlaybackInFlight && mc.isLinkedMediaPlaybackInFlight()
        const mediaRequested = !midiRouteActive && (
            mc.requestedPlayState === 'playMedia' || mc.playbackRouteMode === 'media'
        )
        ...
        if (mc.isMidiPlaybackRoute && mc.isMidiPlaybackRoute()) {
            ...
            if (mc.isPlaying !== isLastPlaying) {
                ...
                if (!nativeOwnsOutput) {
                    if (mc.isPlaying) {
                        if (getForceStop() || playbackFinishedRef.current) {
                            midiStartHandledRef.current = false
                        } else if (midiStartHandledRef.current || isSynthSeekGuardActive()) {
                            midiStartHandledRef.current = false
                        } else {
                            startPlayingFromIntent()
                        }
    ...
    function getMidiPlaybackSeconds() {
        // Prefer the audio-context-anchored clock so the playhead (and mirrored
        // notation cursor) advance between beatCallback ticks. Falling back to
        // the beat-quantized currentTime.current left 2/4 cursors up to half a
        // bar behind the audible notes until the next beat fired.
    ...
    useLayoutEffect(function() {
        ...
        assignMediaControllerRef(mc, 'abortMidiEnginesRef', abortMidiEngines)
        ...
        if (mc.flushPendingPlayRequest) {
            consumedPending = mc.flushPendingPlayRequest()
        }
        if (!consumedPending && mc.kickPlaybackAfterEngineReady) {
        ...
        assignMediaControllerRef(mc, 'getMidiCursorSecondsRef', getCursorMusicSeconds)
        assignMediaControllerRef(mc, 'getAudibleMsPerMeasureRef', function() {
            return getEffectiveMsPerMeasure()
        })
    ...
    function resolveDisplayTuneId() {
        if (props.displayTuneId != null) return props.displayTuneId
        if (props.mediaController && props.mediaController.tune && props.mediaController.tune.id != null) {
            return props.mediaController.tune.id
        }
        return null
    }

    function canMirrorMidiPlaybackCursor() {
        return shouldMirrorMidiPlaybackCursor({
            mirrorNotationPlaybackCursor: props.mirrorNotationPlaybackCursor,
            playbackEngine: props.playbackEngine,
            mediaController: props.mediaController,
            displayTuneId: resolveDisplayTuneId(),
        })
    }

    function ensureMirrorTimingCallbacks() {
        if (!canMirrorMidiPlaybackCursor()) return false
        const visual = gvisualObj.current
        if (!visual) return false
        if (gtimingCallbacks.current && gtimingCallbacks.current.__mirrorVisual === visual) {
            return true
        }
        try {
    ...
    function syncMirroredPlaybackCursor() {
        if (!canMirrorMidiPlaybackCursor()) return
        if (!ensureMirrorTimingCallbacks()) return
        const mc = props.mediaController
        ...
        let audibleMpm = 0
        if (mc && mc.getAudibleMsPerMeasureRef
            && typeof mc.getAudibleMsPerMeasureRef.current === 'function') {
          audibleMpm = parseFloat(mc.getAudibleMsPerMeasureRef.current()) || 0
        }
        if (!(audibleMpm > 0)) {
          const visual = gvisualObj.current
          audibleMpm = (visual && typeof visual.millisecondsPerMeasure === 'function')
        ...
        // mirrored staff line in view (needed when fit-height is off, e.g. page stack).
        if (autoScroll.current && cursor) {
          scrollPlaybackCursorIntoTopHalf(cursor, {
            isPlaying: true,
            noteTimings: timing.noteTimings,
            currentTimeMs: playbackSec * 1000,
            audibleMsPerMeasure: audibleMpm,
          })
        }
    }
    ...
    // Keep the engine cursor on the audio clock between TimingCallbacks beats.
    // Without this, rAF drift leaves the line on the previous bar until the next
    // beatCallback fires.
    useLayoutEffect(function() {
        if (props.playbackEngine === false) return undefined
        if (props.mirrorNotationPlaybackCursor) return undefined
        function tick() {
            if (!(isPlayingRef.current || isMidiPlaybackActive())) return
     ...
     function rhythmPlaySlotFn(audioContext, audioTime, rhythm, slotInBar, destination, meta) {
        ...
        if (phase === PHASE_ENTRY_GAP
            && meta && meta.globalSlot != null && meta.globalSlot >= 0) {
            return
        }
        const nowMusic = getRhythmMusicSeconds()
     ...
     function getEffectiveMsPerMeasure(visualObj) {
        if (effectiveMsPerMeasureRef.current > 0) {
            return effectiveMsPerMeasureRef.current
        }
        if (primedMsPerMeasureRef.current > 0) {
            return primedMsPerMeasureRef.current
        }
        const o = visualObj || gvisualObj.current
        if (o && o.millisecondsPerMeasure) {
            return o.millisecondsPerMeasure()
        }
        return 0
     }
     ...
     function getWallRhythmMetronomeTempo(visualObj) {
        ...
        const factor = getTempoFactor()
        return computeRhythmGridTempo({
            rhythmBeatsPerBar: rhythm.beatsPerBar,
            millisecondsPerMeasure: msPerMeasure,
            tempoFactor: factor > 0 ? factor : 1,
            fallbackQpm: fallback,
        })
     }
     ...
     function getRhythmMusicSeconds() {
        ...
        let fromAnchor = null
        if (anchor.active && gaudioContext.current) {
            const elapsed = gaudioContext.current.currentTime - anchor.audioContextTime
            if (elapsed >= 0 && elapsed < 30) {
                const tempoFactor = getTempoFactor()
                fromAnchor = anchor.musicSeconds + elapsed * (tempoFactor > 0 ? tempoFactor : 1)
            } else {
                fromAnchor = anchor.musicSeconds
            }
        }
        const controller = getRhythmController()
        if (controller.timeline && getRhythmPlaybackPhase(controller) === PHASE_PLAYING) {
            if (pitchShifterRef.current && pitchShifterRef.current.isConnected()) {
                try {
                    const secs = pitchShifterRef.current.getCurrentTime()
        ...
        if (pitchShifterRef.current && pitchShifterRef.current.isConnected()) {
            try {
                const secs = pitchShifterRef.current.getCurrentTime()
                if (Number.isFinite(secs) && secs >= 0) {
                    // SoundTouch can still report 0 briefly after a pre-scheduled downbeat.
                    if (fromAnchor != null && fromAnchor > secs + 0.02) {
                        return fromAnchor
                    }
                    return secs
                }
        ...
        if (isMidiAudioEngineRunning() && typeof currentTime.current === 'number'
            && currentTime.current >= 0) {
     ...
     /**
      * Music seconds for the red cursor. Prefer the audible buffer position and
      * never TimingCallbacks-derived currentTime when voltas are expanded — that
     ...
     function getCursorMusicSeconds() {
        ...
        if (shifterLive) {
            try {
                const s = shifter.getCurrentTime()
                if (Number.isFinite(s) && s >= 0) {
                    return s
                }
            } catch (e) {}
        }
        const fromRhythm = getRhythmMusicSeconds()
        if (Number.isFinite(fromRhythm) && fromRhythm >= 0
            && soundingWrittenMapRef.current
            && soundingWrittenMapRef.current.segments
            && soundingWrittenMapRef.current.segments.length) {
            return fromRhythm
        }
        if (typeof currentTime.current === 'number' && currentTime.current >= 0) {
            return currentTime.current
        }
        return fromRhythm >= 0 ? fromRhythm : 0
     }

     function rebuildPlaybackTimingMap(options) {
        const opts = options || {}
        const visual = gvisualObj.current
        if (!visual) {
            playbackTimingMapRef.current = null
            lastPlaybackTimingSampleRef.current = null
            return
        }
        const bufDur = opts.bufferDuration != null
          ? opts.bufferDuration
          : (gmidiBuffer.current && gmidiBuffer.current.duration)
        const scoreMs = getMsPerMeasureForRhythmGrid(visual)
        let mapVisual = visual
        try {
            mapVisual = expandVisualObjThroughVoltaRepeats(visual) || visual
        } catch (e) {
            mapVisual = visual
        }
        playbackTimingMapRef.current = buildPlaybackTimingMap(mapVisual, {
     ...
     function resolveRhythmBeatBpmFromMap(at) {
        ...
        return at.rhythmBeatBpm
     }

     function syncPlaybackTimingFromMap(force) {
        const map = playbackTimingMapRef.current
        if (!map) return
        if (getRhythmPlaybackPhase(getRhythmController()) !== PHASE_PLAYING) return
        const musicSeconds = getRhythmMusicSeconds()
        const at = timingAtMusicSeconds(map, musicSeconds)
        const targetRhythmBpm = resolveRhythmBeatBpmFromMap(at)
        ...
        const tempoChanged = !!force
            || !last
            || Math.abs(last.rhythmBeatBpm - targetRhythmBpm) > 0.5
        if (tempoChanged) {
            setRhythmPlaybackTempo(controller, targetRhythmBpm)
        }
        if (meterChanged) {
            setRhythmPlaybackRhythm(controller, at.rhythm)
        }
     ...
     function ensureDuringPlaybackMetronome(ratio) {
        ...
        enterRhythmPlaying(controller, {
            ...
            tempo: getWallRhythmMetronomeTempo(),
     ...
     function abortMidiEngines() {
        invalidatePendingMidiStarts()
        forceDestroyAudioEngines()
     }
     ...
     function getRepeatGapMs() {
        ...
        const o = gvisualObj.current
        const msPerMeasure = getEffectiveMsPerMeasure(o)
        if (!o || !o.getBeatsPerMeasure || !(msPerMeasure > 0)) return 0
     ...
     function getMidiStartPosition() {
        ...
        if (props.mediaController) {
            ...
            if (hasPendingStart) {
                ...
                // setClickSeek is async React state and often lags rewind-to-start.
                // Prefer the synchronous currentTimeRef when it says we are at 0.
                const timeRefSeconds = props.mediaController.currentTimeRef
                    ? parseFloat(props.mediaController.currentTimeRef.current)
                    : NaN
                const timeRefAtStart = !isNaN(timeRefSeconds) && timeRefSeconds <= tol
                if (timeRefAtStart && clickSeek > tol) {
                    ratio = 0
                    seconds = 0
                } else {
                    ratio = clickSeek
     ...
     function startPlayingFromIntent(force) {
        ...
        if (!force && (getForceStop() || playbackFinishedRef.current)) {
            return
        }
    ...
    function shouldRestartMidiFromStart(forceRestart) {
     ...
     function createCursor() {
        const root = inputEl && inputEl.current;
        const svg = root ? root.querySelector('svg') : null;
        const cursor = ensureAbcjsCursorLine(svg, gcursor.current)
        if (cursor) {
          setCursor(cursor)
          return cursor
        }
    ...
    /**
     * Place the red cursor from the audio-anchored music clock (not TimingCallbacks
     * rAF wall time). TimingCallbacks free-runs on the *written* score, which is
     * shorter than volta-expanded audio — that made the cursor plow through
     * endings once while playback repeated, and drift ahead of the beat.
     */
    function syncEnginePlaybackCursor() {
        if (props.playbackEngine === false) return
        if (!shouldDrawPlaybackCursor({
            suppressPlaybackVisuals: props.suppressPlaybackVisuals,
        })) return
        const timing = gtimingCallbacks.current
        if (!timing || !timing.noteTimings) return
        const musicSec = getCursorMusicSeconds()
        const audibleMpm = getEffectiveMsPerMeasure()
        const map = soundingWrittenMapRef.current
        let audioDurationSec = (gmidiBuffer.current && gmidiBuffer.current.duration > 0)
          ? gmidiBuffer.current.duration
          : ((props.mediaController && props.mediaController.duration > 0)
            ? parseFloat(props.mediaController.duration)
            : 0)
        if (!(audioDurationSec > 0) && map && map.expectedDurationSec > 0) {
          audioDurationSec = map.expectedDurationSec
        }
        // Do NOT setTimingProgressFromAudioRatio on every tick: seeking
        // TimingCallbacks while it is running fires beat storms and races the
        // cursor. Seek/start paths already sync progress; the cursor reads the
        // audio clock + sounding map directly.
        const root = inputEl && inputEl.current
        const svg = root ? root.querySelector('svg') : null
        if (!svg) return
        const latencyWall = (pitchShifterRef.current
            && typeof pitchShifterRef.current.getOutputLatencySec === 'function'
            && pitchShifterRef.current.isConnected
            && pitchShifterRef.current.isConnected())
          ? (parseFloat(pitchShifterRef.current.getOutputLatencySec()) || 0)
          : 0
        const tempoFactor = getTempoFactor() > 0 ? getTempoFactor() : 1
        // SoundTouch output buffer delays heard notes vs the music clock; scale
        // wall latency into music seconds when playback tempo ≠ 1.
        const latencyMusic = latencyWall * tempoFactor
        const cursor = applyPlaybackCursorAtTime(
            svg,
            gcursor.current,
            timing.noteTimings,
            musicSec * 1000,
            {
              ...
              pickupWhole: map
                ? map.pickupWhole
                : (gvisualObj.current && typeof gvisualObj.current.getPickupLength === 'function'
                  ? parseFloat(gvisualObj.current.getPickupLength()) || 0
                  : 0),
            }
        )
    ...
    function beatCallback(currentBeat,totalBeats,lastMoment,position, debugInfo) {
        syncEnginePlaybackCursor()
        if ((isPlayingRef.current || isMidiPlaybackActive()) && autoScroll.current) {
            scrollPlaybackCursorIntoTopHalf(gcursor.current, {
              isPlaying: true,
              noteTimings: gtimingCallbacks.current && gtimingCallbacks.current.noteTimings,
              currentTimeMs: getCursorMusicSeconds() * 1000,
              audibleMsPerMeasure: getEffectiveMsPerMeasure(),
            })
        }
        // Fall back to abcjs beat position only when timings are unavailable.
        if (shouldDrawPlaybackCursor({
            suppressPlaybackVisuals: props.suppressPlaybackVisuals,
        }) && !(gtimingCallbacks.current && gtimingCallbacks.current.noteTimings)
            && position && position.left != null) {
          if (!gcursor.current) {
            createCursor()
          }
          updateAbcjsCursorLine(gcursor.current, position, false)
        }
        if (shouldDrawPlaybackCursor({
            suppressPlaybackVisuals: props.suppressPlaybackVisuals,
        })) {
        ...
        const hasEngineBuffer = !!(gmidiBuffer.current && gmidiBuffer.current.duration > 0 && totalBeats)
        if (isPlaybackEngine && hasEngineBuffer) {
        // When voltas are expanded, TimingCallbacks walks the *written* score once.
        // Never stretch that progress onto the expanded buffer — it races the cursor
        // through endings while audio is still on early passes. Prefer the real
        // audio / rhythm clock; keep TimingCallbacks synced via sounding→written.
        const hasSoundingMap = !!(soundingWrittenMapRef.current
            && soundingWrittenMapRef.current.segments
            && soundingWrittenMapRef.current.segments.length)
        let newSeconds
        if (hasSoundingMap) {
            newSeconds = getCursorMusicSeconds()
            if (!(newSeconds >= 0) || !isFinite(newSeconds)) {
                newSeconds = getMidiPlaybackSeconds()
            }
            // Do not seek TimingCallbacks from every beat — that storms callbacks
            // and races the cursor. Progress is set on seek/start only.
        } else {
            newSeconds = timingProgressToAudioSeconds(
        ...
        if (!skipPositionUpdate) {
            const anchor = rhythmMusicAnchorRef.current
            let anchorAheadSecs = null
            if (anchor.active && gaudioContext.current) {
                const elapsed = gaudioContext.current.currentTime - anchor.audioContextTime
                const factor = getTempoFactor() > 0 ? getTempoFactor() : 1
                if (elapsed >= 0 && elapsed < 30) {
                    anchorAheadSecs = anchor.musicSeconds + elapsed * factor
                } else {
                    anchorAheadSecs = anchor.musicSeconds
                }
            }
            // With a sounding map, never let TimingCallbacks-derived time outrun
            // the audio anchor (that caused growing lead through volta passes).
            const useSeconds = hasSoundingMap
                ? newSeconds
                : ((anchorAheadSecs != null && anchorAheadSecs > newSeconds + 0.02)
                    ? anchorAheadSecs
                    : newSeconds)
            if (props.mediaController && props.mediaController.onAbcTimeUpdate) {
                props.mediaController.onAbcTimeUpdate(useSeconds)
            }
            currentTime.current = useSeconds
            if (hasSoundingMap || anchorAheadSecs == null || anchorAheadSecs <= newSeconds + 0.02) {
                updateRhythmMusicAnchor(useSeconds)
            }
            maybeStopNotationSelectionPlayback(useSeconds)
            if (getRhythmPlaybackPhase(getRhythmController()) === PHASE_PLAYING) {
                syncPlaybackTimingFromMap(false)
            } else {
                reconcileDuringPlaybackRhythm()
            }
        }
        if (props.onPracticeBeat) {
            const latencyInfo = playalongOutputLatencyInTimelineRef.current || {}
            const shifterLatency = pitchShifterRef.current
                && typeof pitchShifterRef.current.getOutputLatencySec === 'function'
                ? pitchShifterRef.current.getOutputLatencySec()
                : 0
            const ctx = gaudioContext.current
            const deviceLatency = (ctx
                ? (Math.max(0, parseFloat(ctx.outputLatency) || 0)
                    + Math.max(0, parseFloat(ctx.baseLatency) || 0))
                : 0)
            const reportedLatency = Math.max(0, deviceLatency)
                + Math.max(0, parseFloat(shifterLatency) || 0)
            props.onPracticeBeat({
                ...
                outputLatencySeconds: reportedLatency,
                outputLatencyAlreadyInTimeline: !!latencyInfo.alreadyInTimeline,
                outputLatencyAppliedSeconds: Number.isFinite(latencyInfo.appliedSeconds)
                    ? latencyInfo.appliedSeconds
                    : 0,
            })
        }
         // FINISHED PLAYBACK — defer until audio is near the buffer end (see
         // handlePlaybackCompletion / isMidiAudioNearNaturalEnd).
    ...
    function eventCallback(ev) {
        ...
        if (playing && autoScroll.current && gmidiBuffer && gmidiBuffer.current && gmidiBuffer.current.duration > 0) { 
          ...
          if (lastScrollTo.current != ev.top) {
            // Playalong hides the source SVG and shows interleaved staff+roll rows —
            // scroll those visible rows, not the invisible note element.
            if (!scrollPlayalongPlayingLineIntoCenter(ev)) {
              const cursorEl = gcursor.current
              const noteEl = ev.elements && ev.elements[0] && ev.elements[0][0]
              if (cursorLineHasActivePosition(cursorEl) || noteEl) {
                const scrolled = scrollPlaybackCursorIntoTopHalf(cursorEl, {
                  isPlaying: true,
                  noteTimings: gtimingCallbacks.current && gtimingCallbacks.current.noteTimings,
                  currentTimeMs: ev.milliseconds,
                  audibleMsPerMeasure: getEffectiveMsPerMeasure(),
                })
                if (!scrolled && noteEl && typeof noteEl.getBoundingClientRect === 'function') {
                  const rect = noteEl.getBoundingClientRect()
                  if (rect && (rect.height > 0 || rect.width > 0)) {
                    try {
                      noteEl.scrollIntoView({ block: 'start', inline: 'nearest', behavior: 'auto' })
                    } catch (err) {
                      noteEl.scrollIntoView(true)
                    }
                  }
                }
              } else if (noteEl && typeof noteEl.scrollIntoView === 'function') {
                const block = autoScrollBlock.current === 'center' ? 'center' : 'nearest'
                try {
                  noteEl.scrollIntoView({ block: block, inline: 'nearest' })
                } catch (err) {
                  noteEl.scrollIntoView(true)
                }
              } else if (Number.isFinite(ev.top)) {
      ...
      if (!(duration > 0) || !audioBuffers || !audioBuffers[0]) return null
      return encodeAudioBufferWithSetting(audioBuffers[0]).then(function (encoded) {
        return store.setItem(tuneId, [duration, encoded.blob, encoded.format] ).then(function () {
    ...
    function getPlaybackMetronomeTempo(visualObj) {
        const o = visualObj || gvisualObj.current
        const msPerMeasure = getEffectiveMsPerMeasure(o)
        if (!o || !o.getBeatsPerMeasure || !(msPerMeasure > 0)) {
    ...
    function getExtraMeasuresAtBeginning(visualObj) {
        ...
        const metro = resolvePlaybackMetronomeOptions()
        const msPerMeasure = getEffectiveMsPerMeasure(o)
        if (!metro.metronomeCountIn || !o || !(msPerMeasure > 0)) return 0
    ...
    function setTimingProgressFromAudioRatio(audioRatio) {
        ...
        const map = soundingWrittenMapRef.current
        const buf = gmidiBuffer.current
        const r = Math.max(0, Math.min(1, parseFloat(audioRatio) || 0))
        // Prefer sounding→written so TimingCallbacks land on the volta pass that
        // matches expanded audio (not compressed into the written lastMoment end).
        if (map && Array.isArray(map.segments) && map.segments.length
            && buf && buf.duration > 0 && lastMoment > 0) {
            const soundingSec = r * buf.duration
            let soundingWhole = 0
            const audibleMpm = getEffectiveMsPerMeasure()
            ...
            if (audibleMpm > 0 && barWhole > 0) {
                const secPerWhole = (audibleMpm / 1000) / barWhole
                soundingWhole = secPerWhole > 0 ? soundingSec / secPerWhole : 0
            } else if (map.soundingWhole > 0) {
                soundingWhole = soundingSec * (map.soundingWhole / buf.duration)
            }
            const mapped = mapSoundingWholeToWritten(map.segments, soundingWhole)
            if (mapped && map.writtenWhole > 0) {
                const writtenFrac = Math.max(0, Math.min(1, mapped.writtenWhole / map.writtenWhole))
                const musicStart = getTimingMusicStartMs()
                const progress = (musicStart > 0 && musicStart < lastMoment)
                  ? (musicStart + writtenFrac * (lastMoment - musicStart)) / lastMoment
                  : writtenFrac
                try {
    ...
    function initPitchShifter(audioContext, audioBuffer) {
        ...
        pitchShifterRef.current = new PitchTempoShifter(
            ...
            function onEnded() {
                // Near-end reconnect loops refresh the 3s guard and previously
                // swallowed completion — play/pause spasmed and the queue never
                // advanced. Always honor a natural end once past ~99% / <150ms left.
                const dur = pitchShifterRef.current && pitchShifterRef.current.duration > 0
                    ? pitchShifterRef.current.duration
                    : (gmidiBuffer.current && gmidiBuffer.current.duration > 0
                        ? gmidiBuffer.current.duration
                        : 0)
                const pos = pitchShifterRef.current
                    && typeof pitchShifterRef.current.getCurrentTime === 'function'
                    ? pitchShifterRef.current.getCurrentTime()
                    : currentTime.current
                const nearEnd = dur > 0 && (pos / dur >= 0.99 || (dur - pos) < 0.15)
                if (!nearEnd && Date.now() < midiPlaybackGuardUntilRef.current) {
                    return
                }
                if (!nearEnd && isSynthSeekGuardActive()) {
                    return
                }
                midiPlaybackGuardUntilRef.current = 0
                handlePlaybackCompletion()
    ...
    function applyMidiPlaybackSettings(tempo, pitch, fineTune, options) {
        ...
        // Live pitch/fine-tune: retune the running shifter in place. Recreating
        // or calling startMidiAudioOutput here disconnects audio on every tick.
        if (wasPlaying && !tempoChanged && (pitchChanged || fineTuneChanged)) {
            ...
            midiPlaybackGuardUntilRef.current = Date.now() + 3000
            pitchShifterRef.current.applySettings(settings.tempo, settings.pitch, settings.fineTune)
            const connectedOrPending = function() {
                return pitchShifterRef.current.isConnected()
                    || (typeof pitchShifterRef.current.isConnectedOrPending === 'function'
                        && pitchShifterRef.current.isConnectedOrPending())
            }
            if (connectedOrPending()) {
                return
            }
            // Mode switch should leave the shifter connected. Soft-reconnect in
            // place — never rebuild via startMidiAudioOutput on pitch ticks.
            if (pitchShifterBufferRef.current) {
                try {
                    if (typeof ratio === 'number' && ratio >= 0) {
                        pitchShifterRef.current.seek(ratio)
                    }
    ...
    function startPlaying(force = false) {
        ...
        // Forced restarts (play-along, From start) must cancel an in-flight
        // count-in / handoff; otherwise this returns and leaves mid-tune audio.
        if (force) {
            countInPendingRef.current = false
            forceDestroyAudioEngines()
        }
        ...
        if (!force && (countInPendingRef.current || isRhythmHandoffPhase())) {
            return
        }
        if (!force && midiPrimeInFlightRef.current) {
            if (primePromiseRef.current || countInPendingRef.current || isRhythmHandoffPhase()) {
                return
            }
            midiPrimeInFlightRef.current = false
        ...
        if (!force && primePromiseRef.current && isLoading.current) {
            return
        }
        if (force) {
            clearPlaybackFinished()
        ...
        midiPrimeInFlightRef.current = true
        resumeSynthAudioContext()
        const wantMelodyOnly = !!props.forceFillOff
        const primedMelodyOnly = lastPrimedMelodyOnlyRef.current
        if (gaudioContext.current && gmidiBuffer.current
            && primedMelodyOnly != null
            && wantMelodyOnly !== !!primedMelodyOnly) {
        ...
        if (gaudioContext.current && gmidiBuffer.current) {
          startPrimedTune(force)
            ...
            createPlayer(tune, gvisualObj.current, { showUiLoading: true, forcePrime: !!force }).then(function(p) {
                 ...
                 if (!midiBuffer) {
                   ...
                   midiPrimeInFlightRef.current = false
                   if (getForceStop()
                       || shouldQuietMidiPrimeFailure()
                       || !wantsMidiPlayback(true)
                       || primePromiseRef.current) {
                     releaseMidiUiLoading()
                     return
                   }
                   reportMidiPlaybackFailure()
                   return
                 }
    ...
    function beginMidiPlayback(options) {
        ...
        if (props.mediaController && props.mediaController.hasPlayingIntent
            && props.mediaController.hasPlayingIntent()) {
            clearPlaybackFinished()
        }
        let opts = options || {}
        if (opts.fromStart) {
            opts = Object.assign({}, opts, {
                ...
                resume: false,
                alwaysFromSelection: false,
                fromNotationSelection: false,
                startBeat: undefined,
                startMs: undefined,
            ...
            currentTime.current = 0
            if (props.mediaController) {
                if (props.mediaController.currentTimeRef) {
                    props.mediaController.currentTimeRef.current = 0
                }
                if (props.mediaController.setCurrentTime) props.mediaController.setCurrentTime(0)
                if (props.mediaController.setClickSeek) props.mediaController.setClickSeek(0)
                if (props.mediaController.notationPlaybackSeekRef) {
                    props.mediaController.notationPlaybackSeekRef.current = null
                }
                if (props.mediaController.notationPlaybackStartSecondsRef) {
                    props.mediaController.notationPlaybackStartSecondsRef.current = null
                }
            }
        }
        ...
        if (opts.restart) {
            // Keep pendingRestartFromZeroRef armed for fromStart until audio
            // actually begins — clearArmPlaybackFromZero runs after start.
            if (!opts.fromStart && !pendingRestartFromZeroRef.current) {
        ...
        if (fromBeginning) {
            if (opts.restart) {
                // Cancel count-in / stuck kickoff, but keep a good primed buffer so
                // replay after natural end does not re-prime (and toast on race).
                countInPendingRef.current = false
                countInMidiPreScheduledRef.current = false
                if (primeTimerRef.current) {
                    clearTimeout(primeTimerRef.current)
                ...
                if (!(gmidiBuffer.current && gtimingCallbacks.current)) {
                    invalidatePendingMidiStarts()
                } else {
                    stopMetronome()
                    midiPrimeInFlightRef.current = false
            ...
            if (isMidiKickoffActive()) {
                if (!opts.restart
                    ...
                    if (props.mediaController && props.mediaController.pendingMidiPlayRef) {
                        props.mediaController.pendingMidiPlayRef.current = opts
                    }
                    return true
                }
                clearStaleMidiKickoffLock()
            }
            if (gaudioContext.current && gmidiBuffer.current && gtimingCallbacks.current
                && wantsMidiPlayback(true)) {
                if (props.mediaController && props.mediaController.pendingMidiPlayRef) {
                    props.mediaController.pendingMidiPlayRef.current = null
                }
    ...
    function assignStateOnCompletion(audioContext, midiBuffer, timingCallbacks, cursor) {
         ...
         if (midiBuffer && midiBuffer.duration > 0) { 
           ...
           try {
             rebuildPlaybackTimingMap({ bufferDuration: midiBuffer.duration })
           } catch (e) {}
    ...
    function forceDestroyAudioEngines() {
        effectiveMsPerMeasureRef.current = 0
        primedMsPerMeasureRef.current = 0
        lastPrimedMelodyOnlyRef.current = null
        lastPrimedMidiTransposeRef.current = null
        lastPrimedAudioHashRef.current = null
        soundingWrittenMapRef.current = null
        if (props.mediaController) {
    ...
    function seekMidiPlayback(ratio, options) {
        ...
        const allowAutoResume = wasPlaying && !opts.skipAutoResume && !getForceStop()
        midiPlaybackGuardUntilRef.current = Date.now() + 3000
        ...
        try { syncEnginePlaybackCursor() } catch (e) {}
        if (allowAutoResume) {
            const settings = pitchTempoSettingsRef.current
            if (pitchShifterRef.current) {
                if (gaudioContext.current && gaudioContext.current.state === 'running') {
    ...
    function resumeMidiAfterSeek() {
        if (playbackFinishedRef.current || getForceStop()) return
        if (shouldDeferSynthStopToNative()) {
            if (props.mediaController && props.mediaController.confirmPlayingStarted) {
                props.mediaController.confirmPlayingStarted()
            }
        ...
        const ratio = getMidiPlaybackRatio()
        // Do not revive a tune that already finished — that caused ~200ms
        // reconnect spasms at ratio≈0.998 and blocked queue advance.
        if (ratio >= 0.99) {
            return
        }
        midiPlaybackGuardUntilRef.current = Date.now() + 3000
  ...
  async function startMidiAndTiming(startOptions) {
      ...
      const countInHandoffPending = typeof opts.startAtAudioTime === 'number'
      const deferTimingUntilHandoff = countInHandoffPending
          && duringPlayback
          && rhythmPhaseAtStart === PHASE_ENTRY_GAP
      if (!isRhythmHandoffPhase() && !rhythmAlreadyPlaying && !countInPendingRef.current) {
          stopMetronome()
      }
      ...
      try {
          ...
          if (!audioStart || !audioStart.ok) {
              // Primed buffer may be unusable after stop(); re-prime once instead of toasting.
              if (!midiAudioRestartAfterFailRef.current
                  && wantsMidiPlayback(true)
                  && !getForceStop()
                  && !shouldQuietMidiPrimeFailure()) {
                  midiAudioRestartAfterFailRef.current = true
                  startPlaying(true)
                  return false
              }
              midiAudioRestartAfterFailRef.current = false
              handleMidiAudioStartFailure()
              return false
          }
          midiAudioRestartAfterFailRef.current = false
          ...
          if (gtimingCallbacks.current && !deferTimingUntilHandoff) {
          ...
          } else if (deferTimingUntilHandoff) {
              isPlayingRef.current = true
          }
          if (!isPlaybackGenerationCurrent(generation) || !wantsMidiPlayback(forceWanted) || getForceStop()) {
              pauseMidiSynth()
          ...
          if (rhythmHandoffReady && isPlaybackGenerationCurrent(generation)) {
              const controller = getRhythmController()
              // Count-in already built an audio-clock timeline at the correct
              // wall tempo. Do not retarget score BPM here — that made post
              // anacrusis clicks late/slow versus the tune.
              if (!controller.timeline) {
                  syncPlaybackMetronomeTempo()
              }
              const snap = getRhythmTimelineSnapshot(controller)
              ...
              const tempoFactor = getTempoFactor()
              const factor = tempoFactor > 0 ? tempoFactor : 1
              const now = gaudioContext.current ? gaudioContext.current.currentTime : null
              let musicStartAudioTime = scheduledStart
              let musicSeconds = 0
              let forceTimelineReanchor = false
              let handoffMusicSeconds = null
              const alreadyHandedOff = controller.phase === PHASE_PLAYING && !!controller.timeline
              const priorAnchor = rhythmMusicAnchorRef.current
              // True audible MIDI start: prefer the pitch-shifter connect time.
              // When MIDI was pre-scheduled, audioStart.actualStartAudioTime is only
              // the original target — using it re-anchored the click grid backward
              // onto the early schedule and left ~8ms metronome skew.
              let audibleMidiStart = null
              if (pitchShifterRef.current
                  && typeof pitchShifterRef.current.getScheduledConnectTime === 'function') {
                  const sc = pitchShifterRef.current.getScheduledConnectTime()
                  if (typeof sc === 'number') audibleMidiStart = sc
              }
              if (audibleMidiStart == null && !midiWasPreScheduled
                  && audioStart && typeof audioStart.actualStartAudioTime === 'number') {
                  audibleMidiStart = audioStart.actualStartAudioTime
              }
              if (alreadyHandedOff) {
                  const entrySlot = controller.musicStartSlot != null
                      ? controller.musicStartSlot
                      : 0
                  const plannedEntryAudio = controller.timeline
                      ? audioTimeForGlobalSlot(controller.timeline, entrySlot)
                      : (controller.musicStartAudioTime != null
                          ? controller.musicStartAudioTime
                          : scheduledStart)
                  if (audibleMidiStart != null && typeof plannedEntryAudio === 'number'
                      && Math.abs(audibleMidiStart - plannedEntryAudio) > 0.003
                      && Math.abs(audibleMidiStart - plannedEntryAudio) < 0.08
                      && controller.timeline) {
                      ...
                      controller.musicStartAudioTime = audibleMidiStart
                      musicStartAudioTime = audibleMidiStart
                      rhythmMusicAnchorRef.current = {
                          musicSeconds: 0,
                          audioContextTime: audibleMidiStart,
                          active: true,
                      }
                  } else if (!priorAnchor.active) {
                      musicStartAudioTime = controller.musicStartAudioTime != null
                          ? controller.musicStartAudioTime
                          : scheduledStart
                      rhythmMusicAnchorRef.current = {
                          musicSeconds: 0,
                          audioContextTime: typeof musicStartAudioTime === 'number'
                              ? musicStartAudioTime
                              : (now || 0),
                          active: true,
                      }
              ...
              musicStartAudioTime = handoff.actualStartAudioTime
              musicSeconds = handoff.musicSeconds
              handoffMusicSeconds = handoff.musicSeconds
              forceTimelineReanchor = false
              if (audibleMidiStart != null && typeof musicStartAudioTime === 'number'
                  && Math.abs(audibleMidiStart - musicStartAudioTime) > 0.003
                  && Math.abs(audibleMidiStart - musicStartAudioTime) < 0.08) {
                  // Prefer the audible MIDI connect time over the handoff estimate.
                  if (musicSeconds < 0.001) {
                      musicStartAudioTime = audibleMidiStart
                      forceTimelineReanchor = true
                  } else {
                      musicStartAudioTime = audibleMidiStart - musicSeconds / factor
                      forceTimelineReanchor = true
                  }
              }
              // Keep the count-in grid when handoff is on time. Only re-anchor when
              // music audibly started late (SoundTouch connect drift, async handoff).
              if (controller.timeline && now != null) {
                  const scheduledConnect = pitchShifterRef.current
                      && typeof pitchShifterRef.current.getScheduledConnectTime === 'function'
                      ? pitchShifterRef.current.getScheduledConnectTime()
                      : null
                  if (typeof scheduledConnect === 'number' && scheduledConnect > now + 0.001) {
                      musicStartAudioTime = scheduledConnect
                      musicSeconds = 0
                  } else if (handoff.musicSeconds > 0.05) {
                      const actualMusicSeconds = isMidiAudioEngineRunning()
                          ? getRhythmMusicSeconds()
                          : musicSeconds
                      if (actualMusicSeconds > handoff.musicSeconds + 0.02) {
                          musicSeconds = actualMusicSeconds
                          musicStartAudioTime = now - actualMusicSeconds / factor
                          forceTimelineReanchor = true
                      }
                  }
              } else if (isMidiAudioEngineRunning()) {
                  const actualMusicSeconds = getRhythmMusicSeconds()
                  if (actualMusicSeconds > musicSeconds + 0.05 && now != null) {
                      musicSeconds = actualMusicSeconds
                      musicStartAudioTime = now - actualMusicSeconds / factor
                      forceTimelineReanchor = true
                  }
              }
              const anchorAudio = typeof musicStartAudioTime === 'number'
                  ? musicStartAudioTime + musicSeconds / factor
                  : (gaudioContext.current ? gaudioContext.current.currentTime : 0)
              rhythmMusicAnchorRef.current = {
                  musicSeconds: musicSeconds,
                  audioContextTime: anchorAudio,
                  active: true,
              }
              beginRhythmPlayingAtMusicStart(controller, {
                  musicSeconds: musicSeconds,
                  musicStartAudioTime: musicStartAudioTime,
                  forceTimelineReanchor: forceTimelineReanchor,
              ...
              var _map0 = playbackTimingMapRef.current
              ...
              if (deferTimingUntilHandoff && gtimingCallbacks.current) {
                  const bufDur = gmidiBuffer.current && gmidiBuffer.current.duration > 0
                      ? gmidiBuffer.current.duration
                      : 0
                  const handoffProgress = bufDur > 0
                      ? Math.max(0, Math.min(1, musicSeconds / bufDur))
                      : 0
                  setTimingProgressFromAudioRatio(handoffProgress)
                  if (!gtimingCallbacks.current.isRunning) {
                      gtimingCallbacks.current.start()
                  }
              }
          ...
          if (gtimingCallbacks && gtimingCallbacks.current && gmidiBuffer && gmidiBuffer.current) {
              if (!fromStart) {
                ...
                const countInOptions = {
                    ...
                    countInBarOnly: !!metro.countInBarOnly,
                    countInBars: effectiveCountInBars(countInMeter, metro.countInBars),
                    meter: countInMeter,
                    effectiveMsPerMeasure: scoreMsPerMeasure,
                }
                ...
                if (!countInInput) {
                    ...
                    countInInput = {
                        ...
                        beatLength: fallbackBeatLen > 0
                            ...
                            : 0,
                        millisecondsPerMeasure: scoreMsPerMeasure || o.millisecondsPerMeasure(),
                        tempoFactor: tempoFactor,
                        countInBeats: metro.countInBeats,
                        countInBarOnly: !!metro.countInBarOnly,
                        countInBars: effectiveCountInBars(countInMeter, metro.countInBars),
                ...
                const tunePickupLength = parseFloat(o.getPickupLength()) || 0
                const countInPickupLength = metro.countInBarOnly
                    ? 0
                    : tunePickupLength
                // Rhythm-beat units (e.g. 1.0 for a quarter anacrusis in 3/4).
                ...
                function startWithMetronome() {
                    ...
                    const beginCountIn = function(contextReady) {
                        ...
                        if (!(effectiveMsPerMeasureRef.current > 0)
                            && primedMsPerMeasureRef.current > 0) {
                            effectiveMsPerMeasureRef.current = primedMsPerMeasureRef.current
                        }
                        ...
                        // Count-in is scheduled on the audio clock, so use wall BPM
                        // (score BPM × playback speed). During-playback uses score BPM
                        // + tempoFactor separately.
                        if (!(effectiveTempo > 0)) {
                            const baseGrid = getRhythmGridMetronomeTempo()
                            const factor = getTempoFactor()
                            effectiveTempo = baseGrid > 0 ? baseGrid * (factor > 0 ? factor : 1) : 0
                        }
                        ...
                        var countInSlots = resolveCountInSlotCount(
                            o,
                            countInRhythm,
                            countInOptions,
                            metro
                        ...
                        const launchCountIn = function() {
                            ...
                            const countInStarted = startRhythmCountIn(getRhythmController(), {
                                ...
                                onFirstNoteSchedule: (!duringPlayback && shouldScheduleCountInPitchCue())
                                    ? function(time) {
                                    if (!isPlaybackGenerationCurrent(countInGeneration)) return
                                    ...
                                    : null,
                                onPreScheduleMusic: duringPlayback
                                    ? function(musicStartAudioTime) {
                                    if (!isPlaybackGenerationCurrent(countInGeneration)) return
                                    if (!wantsMidiPlayback(true)) return
                                    if (!(typeof musicStartAudioTime === 'number')) return
                                    const midiOut = startMidiAudioOutput(
                                        pitchTempoSettingsRef.current,
                                        0,
                                        musicStartAudioTime
                                    )
                                    countInMidiPreScheduledRef.current = !!(midiOut && midiOut.ok)
                                }
                                    : null,
                                onMusicStart: function(scheduledMusicStartAudioTime) {
                                    ...
                                    if (duringPlayback && startAt != null) {
                                        // Prefer the count-in pre-schedule; only arm here if
                                        // that path missed (reuse guard avoids tearing it down).
                                        const midiOut = startMidiAudioOutput(
                                            pitchTempoSettingsRef.current,
                                            0,
                                            startAt
                                        ...
                                        const controller = getRhythmController()
                                        const snap = getRhythmTimelineSnapshot(controller)
                                        const handoffNow = gaudioContext.current
                                            ? gaudioContext.current.currentTime
                                            : null
                                        const outputLatencySec = pitchShifterRef.current
                                            && typeof pitchShifterRef.current.getOutputLatencySec === 'function'
                                            ? pitchShifterRef.current.getOutputLatencySec()
                                            : 0
                                        const scheduledConnect = pitchShifterRef.current
                                            ...
                                            ? pitchShifterRef.current.getScheduledConnectTime()
                                            : null
                                        const midiStart = (typeof scheduledConnect === 'number'
                                            ? scheduledConnect
                                            : (midiOut && typeof midiOut.actualStartAudioTime === 'number'
                                                ? midiOut.actualStartAudioTime
                                                : handoffNow))
                                        const audibleMusicStart = typeof midiStart === 'number'
                                            ? midiStart + Math.max(0, outputLatencySec)
                                            : (startAt != null
                                                ? startAt + Math.max(0, outputLatencySec)
                                                : null)
                                        const connectDrift = audibleMusicStart != null && startAt != null
                                            ? audibleMusicStart - (startAt + Math.max(0, outputLatencySec))
                                            : 0
                                        const entrySlot = snap && snap.musicStartSlot != null
                                            ? snap.musicStartSlot
                                            : 0
                                        // Keep count-in phase; shift only when MIDI connects
                                        // measurably late so entry slot matches audible start.
                                        const pickupBeats = controller.pickupBeats || 0
                                        const secPerBeat = controller.tempo > 0 ? 60 / controller.tempo : 0
                                        const fractionalPickup = entrySlot === 0 && pickupBeats > 0
                                            && pickupBeats < 1 - 1e-6 && secPerBeat > 0
                                        const scheduledAnacrusis = startAt != null
                                            ? startAt + Math.max(0, outputLatencySec)
                                            : null
                                        let timelineAnchorSlot = entrySlot
                                        let timelineMusicStart = scheduledAnacrusis != null
                                            ? scheduledAnacrusis
                                            : (startAt != null
                                                ? startAt + Math.max(0, outputLatencySec)
                                                : startAt)
                                        let needsReanchor = outputLatencySec > 0.001
                                        if (fractionalPickup) {
                                            // First note is anacrusis; timeline slot 0 is downbeat.
                                            // Prefer the real audible anacrusis when connect drifted.
                                            const anacrusisTime = (connectDrift > 0.003
                                                && audibleMusicStart != null)
                                                ? audibleMusicStart
                                                : (scheduledAnacrusis != null
                                                    ? scheduledAnacrusis
                                                    : audibleMusicStart)
                                            timelineMusicStart = (anacrusisTime != null
                                                ? anacrusisTime
                                                : timelineMusicStart) + pickupBeats * secPerBeat
                                            timelineAnchorSlot = 0
                                            needsReanchor = true
                                        } else if (connectDrift > 0.003 && audibleMusicStart != null) {
                                            timelineMusicStart = audibleMusicStart
                                            timelineAnchorSlot = entrySlot
                                            needsReanchor = true
                                        }
                                        playalongOutputLatencyInTimelineRef.current = {
                                            alreadyInTimeline: needsReanchor && outputLatencySec > 0.001,
                                            appliedSeconds: needsReanchor && outputLatencySec > 0.001
                                                ? Math.max(0, outputLatencySec)
                                                : 0,
                                        }
                                        // Music seconds 0 = first sounding note (anacrusis),
                                        // not the downbeat used as timelineMusicStart for pickups.
                                        const musicClockAudioTime = (connectDrift > 0.003
                                            && audibleMusicStart != null)
                                            ? audibleMusicStart
                                            : (scheduledAnacrusis != null
                                                ? scheduledAnacrusis
                                        ...
                                        rhythmMusicAnchorRef.current = {
                                            musicSeconds: 0,
                                            audioContextTime: musicClockAudioTime != null
                                                ? musicClockAudioTime
                                                : (audibleMusicStart != null
                                                    ? audibleMusicStart
                                                    : (startAt || 0)),
                                        ...
                                        beginRhythmPlayingAtMusicStart(controller, {
                                            musicSeconds: 0,
                                            musicStartAudioTime: timelineMusicStart,
                                            forceTimelineReanchor: needsReanchor,
                                            timelineAnchorSlot: needsReanchor
                                                ? timelineAnchorSlot
                                                : undefined,
                                            musicStartSlot: snap && snap.musicStartSlot != null
                                                ? snap.musicStartSlot
  ...
  function primeTune(tune, audioContext, visualObj, force = false, options = {}) {
      ...
      if (primePromiseRef.current && isLoading.current && !force) {
          return primePromiseRef.current
      }
      //var tempo = tune ? tune.tempo : 100
      const promise = new Promise(function(resolve,reject) {
          const generation = playbackGenerationRef.current
          const canReuseConcurrentPrime = !force
              && gmidiBuffer.current
              && gtimingCallbacks.current
              && (primePromiseRef.current || countInPendingRef.current || isRhythmHandoffPhase())
          if (canReuseConcurrentPrime) {
            isLoading.current = false
          ...
          if (visualObj) {
            ...
            var count = 0
            var fillTune = props.forceFillOff
              ? Object.assign({}, tune, {
                  playbackFillStyle: FILL_STYLE_OFF,
                  playbackFillLevel: 0,
                })
              : tune
            var fillPlayback = resolveFillPlaybackOptions(fillTune, props.tunebook)
            lastPrimedMelodyOnlyRef.current = !!props.forceFillOff || fillPlayback.settings.style === FILL_STYLE_OFF
            ...
            var effectiveProgramOffsets = programOffsetsForPlaybackPlan(a, soundFontPlan)
            ...
            var initOptions = {
              ...
              options: abcjsPlaybackSynthOptions({
                 ...
                 //program: 21,
                 chordsOff: fillPlayback.chordsOff,
                 programOffsets: effectiveProgramOffsets,
               }),
            }
            var useSequencePath = true
            var audibleMsPerMeasureForPrime = initOptions.millisecondsPerMeasure
            try {
              soundingWrittenMapRef.current = buildSoundingWrittenMap(synthObj)
              if (props.mediaController) {
                assignMediaControllerRef(
                  props.mediaController,
              ...
              if (!flattened || !Array.isArray(flattened.tracks) || !flattened.tracks.length) {
                throw new Error('Playback sequence missing tracks')
              }
              // Stamp expected buffer length so cursor clock matches sequence even
              // before midiBuffer.duration is assigned.
              if (soundingWrittenMapRef.current && audibleMsPerMeasureForPrime > 0) {
                var barW = barWholeNotesFromMeter(
                  typeof synthObj.getMeterFraction === 'function'
                    ? synthObj.getMeterFraction()
                    : null
                )
                if (barW > 0 && soundingWrittenMapRef.current.soundingWhole > 0) {
                  soundingWrittenMapRef.current.expectedDurationSec =
                    soundingWrittenMapRef.current.soundingWhole
                    * (audibleMsPerMeasureForPrime / 1000) / barW
                }
              }
              ...
              delete initOptions.visualObj
              // abcjs CreateSynth defaults meterSize=1 when only `sequence` is
              // passed (visualObj path sets num/den). Scale the init option for
              // CreateSynth, but keep audible bar length for TimingCallbacks /
              // metronome QPM (see resolveSequencePathMeasureTiming).
              var measureTiming = resolveSequencePathMeasureTiming(
                initOptions.millisecondsPerMeasure,
                typeof synthObj.getMeterFraction === 'function'
                  ? synthObj.getMeterFraction()
                  : null
              )
              audibleMsPerMeasureForPrime = measureTiming.audibleMsPerMeasure
              initOptions.millisecondsPerMeasure = measureTiming.createSynthMsPerMeasure
              if (audibleMsPerMeasureForPrime > 0) {
                primedMsPerMeasureRef.current = audibleMsPerMeasureForPrime
                effectiveMsPerMeasureRef.current = audibleMsPerMeasureForPrime
              }
            } catch (remapErr) {
              console.warn('Playback sequence build failed; using visualObj', remapErr)
              soundingWrittenMapRef.current = null
              if (props.mediaController) {
                assignMediaControllerRef(props.mediaController, 'soundingWrittenMapRef', null)
              }
              initOptions.visualObj = synthObj
              delete initOptions.sequence
              useSequencePath = false
            }
            ...
            function getAudioHash(tune) {
              ...
              // are not reused after the millisecondsPerMeasure compensation.
              // Include ofsMusyng0 so MusyngKite buffers rendered with original-bank
              // programOffsets are not reused after offsets were cleared.
              // Include visual/MIDI transpose so capo/transpose display changes
              // do not reuse a buffer primed at a different pitch.
              var meterKey = ''
              try {
                if (synthObj && typeof synthObj.getMeterFraction === 'function') {
                  var mf = synthObj.getMeterFraction()
                  if (mf && mf.num && mf.den) meterKey = mf.num + '_' + mf.den + 'msfix'
                }
              } catch (e) {}
              var seqMsKey = useSequencePath
                ? Math.round(initOptions.millisecondsPerMeasure || 0)
                : 0
              var ofsKey = Object.keys(effectiveProgramOffsets || {}).length ? 'ofs1' : 'ofsMusyng0'
              var bankKey = soundFontPlan.bank === 'online' ? 'online' : 'sel'
              var fillKey = props.forceFillOff
                ? 'off-0'
                : ((tune && tune.playbackFillStyle ? tune.playbackFillStyle : 'boom-chick')
              ...
              // tempoFix1-rep3: reject duration-mismatched caches (pitch-clock + meterSize).
              return tune.id + "-" + tune.tempo  + '-'+transposeKey+"-vtx1-tempoFix1-rep3-"+meterKey+"-"+seqMsKey+"-"+ofsKey+"-"+bankKey+"-"+fillKey+"-"+props.tunebook.utils.hash(props.tunebook.abcTools.getNotesFromAbc(props.abc))
            }
            
            function resolveWithTimingAndCursor(midiBuffer) {
              if (!isPlaybackGenerationCurrent(generation) || primePromiseRef.current !== promise) {
                isLoading.current = false
                if (primePromiseRef.current !== promise) {
                  // Superseded by a forced re-prime; leave loading to the active prime.
                } else {
                  releaseMidiUiLoading()
                }
                resolve(null)
              ...
              lastPrimedAudioHashRef.current = getAudioHash(tune)
              if (midiBuffer && midiBuffer.duration > 0 && props.mediaController) {
                props.mediaController.setDuration(midiBuffer.duration)
              }
              if (midiBuffer && midiBuffer.duration > 0 && soundingWrittenMapRef.current) {
                soundingWrittenMapRef.current.expectedDurationSec = midiBuffer.duration
                if (props.mediaController) {
                  assignMediaControllerRef(
                    props.mediaController,
                    'soundingWrittenMapRef',
              ...
              try {
                rebuildPlaybackTimingMap({ bufferDuration: midiBuffer && midiBuffer.duration })
              } catch (e) {}
              releaseMidiUiLoading()
            ...
            function primeAndResolve() {
                  // Rejected sample fetches are cached forever by abcjs; clear them
                  // (and fully clear on user play) so missing pitches can reload.
                  if (force || showUiLoading) {
                    clearAbcjsSoundsCache()
                  } else {
                    clearRejectedAbcjsSoundsCache()
                  }
                  midiBuffer.init(initOptions).then(
                  function (response) {
                    var loadErrors = response && Array.isArray(response.error) ? response.error : []
                    if (loadErrors.length) {
                      console.warn('Soundfont sample load errors', loadErrors.slice(0, 12))
                      clearRejectedAbcjsSoundsCache()
                    }
                    midiBuffer.prime().then(function(presponse) {
                      resolveWithTimingAndCursor(midiBuffer)
                      if (isPlaybackGenerationCurrent(generation)
                          && tune && tune.id && props.cacheAudio !== false) {
              ...
              var skipAudioCache = fillPlayback.injectCustomFill || force || showUiLoading
              if ((tune && tune.id)) {
                if (props.cacheAudio !== false && !skipAudioCache) {
                    getAudioFromCache(getAudioHash(tune), audioContext).then(function(audioResult) {
                        if (audioResult) {
                          const [cachedDuration, audioBuffers] = audioResult
                          if (audioBuffers && audioBuffers[0]) {
                             const bufferDur = parseFloat(audioBuffers[0].duration) || 0
                             const expectedDur = soundingWrittenMapRef.current
                               && soundingWrittenMapRef.current.expectedDurationSec > 0
                               ? soundingWrittenMapRef.current.expectedDurationSec
                               : 0
                             // Trust the decoded buffer length; drop caches whose
                             // duration disagrees with the expanded-score expectation
                             // (stale meterSize / pitch-clock bugs stretched audio).
                             const dur = bufferDur > 0 ? bufferDur : (parseFloat(cachedDuration) || 0)
                             const mismatch = expectedDur > 0 && dur > 0
                               && Math.abs(dur - expectedDur) / expectedDur > 0.12
                             if (mismatch) {
                               primeAndResolve()
                               return
                             }
                ...
                primeAudio().then(function(audioContext) {
                    if (audioContext) {
                        ...
                        primeTimerRef.current = setTimeout(function() {
                          ...
                          primeTune(tune, audioContext, visualObj, !!options.forcePrime, Object.assign({}, options, {
