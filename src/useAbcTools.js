...
var useAbcTools = () => {
    ...
    function isAbcHeaderMetaValue(value) {
    ...
    function abc2json(abc) {
      ...
      if (abc && abc.trim().length > 0) {
            ...
            if (isCommentLine(line)) {
                ...
                switch (key) {
                    case "W":
                        ...
                        break
                    case "w":
                        if (!Array.isArray(tune.wLines)) tune.wLines = []
                    ...
                    case "C":
                        {
                            ...
                            if (!tune.composer) {
                                ...
                                tune.artists = mergeBibliographicList(tune.artists, composerText)
                            }
                        }
                        break
                    case "G":
                ...
                } else if (line.startsWith('% abcbook-json ')) {
                    abcbookJsonChunks = collectAbcbookJsonChunk(parseAbcbookJsonLine(line), abcbookJsonChunks)
               } else if (line.startsWith('% abcbook-timing-scaffold')) {
        ...
        Object.keys(timedJsonFields).forEach(function(fieldName) {
          if (LEGACY_TIMED_JSON_FIELDS.indexOf(fieldName) >= 0) return
    ...
    function json2abc(tune) {
      ...
      if (tune) {
        normalizeGenre(tune)
        var titleHeaderText = renderBibliographicTitleLines(tune).join("\n")
        ...
        if (Array.isArray(tune.recordings)&& tune.recordings.length > 0) {
            tune.recordings.forEach(function(file,k) {
                if (file.googleDocumentId) {
                    recordingsRendered.push("% abcbook-recording-google-document-id-"+k + ' ' +  ensureText(file.googleDocumentId,"") )
                }
                if (file.type) {
                        recordingsRendered.push("% abcbook-recording-type-"+k + ' ' +  ensureText(file.type,"") )
				}
				if (file.name) {
					recordingsRendered.push("% abcbook-recording-name-"+k + ' ' +  ensureText(file.name,"") )
				}
				if (file.data) {  
					recordingsRendered.push("% abcbook-recording-data-"+k + ' ' +  ensureText(file.data,"") )
				}
            })
        }
  ...
  function tunesToAbc(tunes, deletedTunes) {
    ...
    function tunesToLinkList (tunes) {
        return tunes.map(function(tune) {
            return tune.name  + (tune.composer ? '  -  ' + tune.composer : '') + "\n" + (Array.isArray(tune.links) ? tune.links.map(function(link) {return link.link}).join("\n") : "\n")
        }).join("\n\n")
    }
