...
export default function MidiInputPanel(props) {
  ...
  return (
    <Dropdown as={ButtonGroup} className="notation-midi-dropdown">
      <Button
        size="lg"
        ...
        onClick={async function() {
          ...
          setMidiPatch({ midiEnabled: !session.midiEnabled });
        }}
      >
      ...
      <Dropdown.Toggle split variant="outline-secondary" size="lg" aria-label="MIDI options" />
        ...
        <div className="mb-2">
          <Form.Check
            ...
            onChange={async function(e) {
              if (e.target.checked) await midi.requestAccess();
              setMidiPatch({ midiEnabled: e.target.checked });
            }}
          />
        </div>
