...
export default function Header(props) {
    ...
    return <header className="App-header" style={{zIndex:11}}>
        ...
        {location.pathname.startsWith('/tunes/') && <span style={{float:'right'}} ><TempoControl  tunebook={props.tunebook} value={props.tunebook.tempo} onChange={function(val) {props.tunebook.setTempo(val)}}  /></span>}
    </header>
}
