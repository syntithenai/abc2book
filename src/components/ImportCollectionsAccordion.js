                    ...
                    return <Accordion.Item key={gk}  eventKey={Number(gk)}>
                        ...
                        <Accordion.Body>{groupOptions.map(function(bookTitle,ok) {
                            if (groupItems[bookTitle].link) {
                                ...
                                <Link to={'/importlink/' + encodeURIComponent(baseImportLink + groupItems[bookTitle].link) + (groupItems[bookTitle].book ? "/book/"+encodeURIComponent(groupItems[bookTitle].book) : "") + (groupItems[bookTitle].tag ? "/tag/"+encodeURIComponent(groupItems[bookTitle].tag) : "")} style={{textDecoration:'none'}} >
                                ...
                                &nbsp;{bookTitle}
                                </Button></Link><Link to={'/importlink/' + encodeURIComponent(baseImportLink + groupItems[bookTitle].link) + (groupItems[bookTitle].book ? "/book/"+encodeURIComponent(groupItems[bookTitle].book)+  (groupItems[bookTitle].tag ? "/tag/"+encodeURIComponent(groupItems[bookTitle].tag) : "") + "/play" : "")}  style={{textDecoration:'none'}} ><Button  variant={"primary"} size="small" >{props.tunebook.icons.playwhite}</Button></Link></ButtonGroup>
                    ...
                    {Object.keys(notCollatedCurated).length > 0 && <Accordion.Item eventKey="other">
                         ...
                         <Accordion.Body>{Object.keys(notCollatedCurated).map(function(bookTitle,ok) {
                            if (notCollatedCurated[bookTitle].link) {
                                return <ButtonGroup key={bookTitle} style={buttonGroupStyle} variant="success"><Link to={'/importlink/' + encodeURIComponent(notCollatedCurated[bookTitle].link) + (notCollatedCurated[bookTitle].book ? "/book/"+encodeURIComponent(notCollatedCurated[bookTitle].book) : "")}  style={{textDecoration:'none'}} ><Button onClick={function(e) {props.setCurrentTuneBook(bookTitle)}} >
                                {notCollatedCurated[bookTitle].image ? <img alt="" src={notCollatedCurated[bookTitle].image} style={{height:'80px'}}  /> : null}
                                &nbsp;{bookTitle}
                                </Button>&nbsp;&nbsp;</Link><Link to={'/importlink/' + encodeURIComponent(notCollatedCurated[bookTitle].link) + (notCollatedCurated[bookTitle].book ? "/book/"+encodeURIComponent(notCollatedCurated[bookTitle].book) + "/play" : "")} style={{textDecoration:'none'}} ><Button  variant={"primary"} size="small" >{props.tunebook.icons.playwhite}</Button></Link></ButtonGroup>
                                ...
                                &nbsp;{bookTitle}</Button>&nbsp;&nbsp;</Link><Link to={'/importlink/' + encodeURIComponent(notCollatedCurated[bookTitle].link) + (notCollatedCurated[bookTitle].book ? "/book/"+encodeURIComponent(notCollatedCurated[bookTitle].book) + "/play" : "")} style={{textDecoration:'none'}} ><Button  variant={"primary"} size="small" >{props.tunebook.icons.playwhite}</Button></Link></ButtonGroup>
                            }
                        })}</Accordion.Body>
                     </Accordion.Item>
