                    ...
                    return <Accordion.Item key={gk}  eventKey={Number(gk)}>
                        ...
                        <Accordion.Body>{groupOptions.map(function(bookTitle,ok) {
                            if (groupItems[bookTitle].link) {
                                ...
                                {imageIsHidden[ok] && <img style={{height:'80px'}} src={"data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAAAXNSR0IArs4c6QAAAA1JREFUGFdj4M37+x8ABHwCeNvV2gcAAAAASUVORK5CYII="} />}
                                &nbsp;{bookTitle}</Button></Link><Link  to={'/importlink/' + encodeURIComponent(groupItems[bookTitle].link) + (groupItems[bookTitle].book ? "/book/"+encodeURIComponent(groupItems[bookTitle].book) + "play" : "")} key={'nm-'+ok} style={{textDecoration:'none'}} ><Button  variant={"primary"} size="small" >{props.tunebook.icons.playwhite}</Button></Link></ButtonGroup>
                            }
                        })}</Accordion.Body>
                    </Accordion.Item>})}
