...
export default function FileDrawModal(props) {
  ...
  const inkCanvasGetter = useRef(null)
  ...
  async function flatten() {
    ...
    // Rebuild ink on an offscreen canvas from strokes for export
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (resolverAvailable) {
        ...
        await onSave(blob, { submitOcr: false })
        onHide()
      }
    } finally {
      setSaving(false)
  ...
  async function finishOcrChoice(submit) {
    ...
    setSaving(true)
    try {
      if (submit && onSaveAndOcr) await onSaveAndOcr(blob)
      else if (onSave) await onSave(blob, { submitOcr: false })
      onHide()
    } finally {
      setSaving(false)
    }
  ...
  return (
    <>
      <Modal show={!!show && !askOcr} onHide={onHide} fullscreen className="file-draw-modal">
      ...
      <Modal show={askOcr} onHide={function() { finishOcrChoice(false) }} centered>
        <Modal.Header closeButton>
          <Modal.Title>Submit for OCR review?</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          Send this file for background OCR so you can review suggested edits to this tune?
        </Modal.Body>
        <Modal.Footer>
          <Button variant="secondary" onClick={function() { finishOcrChoice(false) }}>Not now</Button>
          <Button variant="primary" onClick={function() { finishOcrChoice(true) }}>Yes, submit</Button>
        </Modal.Footer>
      </Modal>
    </>
  )
