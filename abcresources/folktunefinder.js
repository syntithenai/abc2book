...
(async () => {
  try {
    ...
    while (a < (start + limit)) {
        // 
        if (continuousErrors < maxErrors && continuousEmpty < maxEmpty &&!fs.existsSync(__dirname + "/abcfiles/" + generateFileName(a)) && !fs.existsSync(__dirname + "/empty/"+generateFileName(a))) {
            try {
                ...
                if (abcBlobs.length  > 0) {
                    ...
                    fs.writeFile(__dirname + "/abcfiles/" + generateFileName(a), results, err => {
                      if (err) {
                        console.error(err)
                        return
                      }
