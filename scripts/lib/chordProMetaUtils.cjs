/** Mirror of src/chordProMetaUtils.js for Node convert script (keep in sync). */
/**
...
function isSectionLabelComment(text) {
 ...
 */
function subtitleLooksLikeArtistCredit(text) {
...
function parseTempoHintFromText(raw) {
 ...
 */
function parseBraceTempoDirective(raw) {
  const text = trimText(raw)
  if (!text) return 0
...
function isBraceTempoDirective(raw) {
 ...
 */
function parseSongDurationSeconds(raw) {
  const text = trimText(raw)
  if (!text) return 0
 ...
 */
function parseCapoFromText(raw) {
  const text = trimText(raw)
 ...
 */
function extractChordProDirectives(text) {
  const found = {}
 ...
 */
function resolveChordProImportMeta(options) {
  const opts = options || {}
 ...
 */
function appendChordProMetaAbcHeaders(lines, meta, options) {
  const draft = meta || {}
  const opts = options || {}
...
module.exports = {
  isSectionLabelComment: isSectionLabelComment,
  subtitleLooksLikeArtistCredit: subtitleLooksLikeArtistCredit,
  parseTempoHintFromText: parseTempoHintFromText,
  parseBraceTempoDirective: parseBraceTempoDirective,
  isBraceTempoDirective: isBraceTempoDirective,
  parseSongDurationSeconds: parseSongDurationSeconds,
  parseCapoFromText: parseCapoFromText,
  extractChordProDirectives: extractChordProDirectives,
  resolveChordProImportMeta: resolveChordProImportMeta,
  appendChordProMetaAbcHeaders: appendChordProMetaAbcHeaders,
};
