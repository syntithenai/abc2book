/* Classic-script xml2abc for EuroSession review_abc.html.
 * Derived from src/xml2abc.js (Willem Vree, rev 118) — no ES modules.
 */
...
Parser.prototype.doNotations = function (note, $nttn, isTab) {
    ...
    if ($wvln.length) {
        ...
        case 'stop': note.before.unshift ('!trill)!'); break;
        }
    }
...
Parser.prototype.tabnote = function (alt, ptc, oct, v, ntrec) {
    ...
    snaar_nw = ntrec.tab [0]
    fret_nw = ntrec.tab [1];  // the computed/annotated allocation of nt
...
Parser.prototype.doNote = function ($n) {
    ...
    note.before = ['']
    note.after = ''     // strings with ABC stuff that goes before or after a note/chord
    ...
    $e.find ('measure-style').each (function () {
        ...
        $(this).find ('measure-repeat').each (function () {
            ...
            if (ty == 'start') {        // remember start measure number and text voor each staff
                ...
                start_ix = dit.repeat_str [n][0]
                text_ = dit.repeat_str [n][1];
...
Parser.prototype.addMidiMap = function (ip, vvmap) {    // map abc voices to midi settings
    ...
    for (i = 0; i < ks.length; ++i) {   // add %%map's for all tab voices
        ...
        v = xs [1]
         noot = xs [2];
        snaar = this.tabmap [v_noot][0]
         fret = this.tabmap [v_noot][1];
...
window.xml2abc_VERSION = xml2abc_VERSION;
